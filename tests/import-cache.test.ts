...
describe('ImportedFileCache', () => {
  ...
  it('forgets bundle bytes and identity metadata together', () => {
    const cache = new ImportedFileCache(4, 1024);
    cache.remember('asset.glb', [file('asset.glb', 10, 1)]);
    cache.forget('asset.glb');
    expect(cache.lookup('asset.glb').status).toBe('missing');
    expect(() => cache.remember('asset.glb', [file('asset.glb', 11, 2)])).not.toThrow();
  });
