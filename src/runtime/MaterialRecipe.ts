...
export const PTL_MATERIAL_VERSION = 1;
...
export function parseMaterialRecipe(value: unknown): MaterialRecipe {
  ...
  if (recipe.format !== PTL_MATERIAL_FORMAT) {
    throw new Error('File is not a Procedural Texture Lab material recipe.');
  }
  if (recipe.version !== PTL_MATERIAL_VERSION) {
...
export function serializeMaterialRecipe(
  ...
  const recipe = 'format' in source
    ? parseMaterialRecipe(source)
    : createMaterialRecipe(source, seed);
  return `${JSON.stringify(recipe, null, 2)}\n`;
}

export async function loadMaterialRecipe(
  input: RequestInfo | URL,
  init?: RequestInit
): Promise<MaterialRecipe> {
  const response = await fetch(input, init);
  if (!response.ok) {
    throw new Error(`Could not load PTL material recipe (${response.status} ${response.statusText}).`);
  }
