...
export function remapGeometryUvToAtlas(
  ...
  if (!Number.isInteger(slot) || slot < 0 || !Number.isInteger(grid) || grid < 1) {
    throw new Error('Invalid shared-atlas slot.');
  }
  ...
  for (let index = 0; index < uv.count; index += 1) {
    uv.setXY(index, (uv.getX(index) + col) / grid, (uv.getY(index) + row) / grid);
  }
  uv.needsUpdate = true;
  return geometry;
}
